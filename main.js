...
while (input !== commands[3]) {
  input = prpmpt('「' + commands + '」の4つのいづれかを入力してください');
  if (input === commands[0]) {
    showTodos();
...
function deleteTodo() {
  ...
  const persedNumber = perseInt(deleteNumber, 10);
  if(isNaN(persedNumber) || persedNumber >= todos.length || persedNumber < 0) {
    alert('不正な値のためスキップします');
  } else {
    const deletedTodos = todos.splice(persedNumber, 1);
